...
def main():
    ...
    while current_iter <= total_iters:
        # Turning augmentaion on for the last few epochs
        if opt['datasets']['train']['finetune_augment']:
            if not augment_on:
                if current_iter >= opt['datasets']['train']['finetune_start_iteration']:
                    augment_on = True
                    print("*****************Aumentaion started***********************")
                    opt['datasets']['train']['use_flip'] = True
                    # create train and validation dataloaders
                    result = create_train_val_dataloader(opt, logger)
                    train_loader, train_sampler, val_loader, total_epochs, total_iters = result
                        # dataloader prefetcher
