...
class BaseModel():
    ...
    def update_learning_rate(self, current_iter, warmup_iter=-1, reset_lr=0):
        ...
        if self.opt['datasets']['train']['finetune_augment']:
            if current_iter > self.opt['datasets']['train']['finetune_start_iteration']:
                lr = self.opt['datasets']['train']['finetune_start_iteration_lr']
                init_lr_g_l = self._get_init_lr()
                warm_up_lr_l = []
                for init_lr_g in init_lr_g_l:
                    warm_up_lr_l.append(
                        [lr for v in init_lr_g])
                self._set_lr(warm_up_lr_l)
