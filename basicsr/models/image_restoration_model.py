...
# ------------------------------------------------------------------------
# Modified NAFNet (https://github.com/megvii-research/NAFNet),
#          Multi Output Deblur (https://github.com/Liu-SD/multi-output-deblur)
# ------------------------------------------------------------------------
import importlib
import torch
...
class MultiHeadImageRestorationModel(BaseModel):
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        ...
        if with_metrics:
            ...
            self.metric_results.update({
                metric+'_best_laplacian_variance': 0
                for metric in self.opt['val']['metrics'].keys()
            })
            self.metric_results.update({
                metric+'_best_sobel': 0
                for metric in self.opt['val']['metrics'].keys()
            })
            self.metric_results.update({
                metric+'_best_scharr': 0
                for metric in self.opt['val']['metrics'].keys()
            })
            self.metric_results.update({
                metric+'_best_tenengrad': 0
                for metric in self.opt['val']['metrics'].keys()
            })
            self.metric_results.update({
                metric+'_best_overall': 0
                for metric in self.opt['val']['metrics'].keys()
            })
            ...
            if with_metrics:
                ...
                if use_image:
                    for name, opt_ in opt_metric.items():
                        ...
                        self.metric_results[name+'_best_sobel'] += _r[3]["sobel"]
                        self.metric_results[name+'_best_scharr'] += _r[3]["scharr"]
                        self.metric_results[name+'_best_tenengrad'] += _r[3]["tenengrad"]
                        self.metric_results[name+'_best_laplacian_variance'] += _r[3]["laplacian_variance"]
                        self.metric_results[name+'_best_overall'] += _r[3]["overall"]
                else:
                    for name, opt_ in opt_metric.items():
                        metric_type = opt_.pop('type')
                        _r = getattr(
